 ...
 */
public class CentralizadorDetallado implements Controller {
                ...
                if (id_curso != null) {
                    ...
                    for (int i = 0; i < estudiantes.size(); i++) {
                        ...
                        for (int j = 0; j < cursomaterias.size(); j++) {
                            ...
                            String id_evaluacion = cursomateria.getId_curso_materia() + "%";
                            Evaluacion evaluacion = new Evaluacion();
                            evaluacion.setId_estudiante(estudiante.getId_estudiante());
                            evaluacion.setId_evaluacion(id_evaluacion);
                            List evaluaciones = this.sigaa.getEvaluacionesByIdestudianteAndIdCursomateria(evaluacion);
                            int dividir_a = 1;
                            int nota = 0;
                            List new_evaluaciones = new ArrayList();
                            int min = 0;
                            if (evaluaciones.size() > 0) {
                                dividir_a = evaluaciones.size();
                                for (int k = 0; k < evaluaciones.size(); k++) {
                                    Evaluacion eval = (Evaluacion) evaluaciones.get(k);
                                    eval.setNotasuma(eval.getNota() + eval.getDps());
                                    nota = nota + eval.getNota() + eval.getDps();
                                    
                                    new_evaluaciones.add(eval);
                                    min = ((eval.getNota_plan() + eval.getDps_plan()) / 2) + 1;
                                }
                            }
                            nota = (int) (Math.round(((double) nota / dividir_a) * Math.pow(10, 0)) / Math.pow(10, 0));
                            ...
                            cm.setCualitativa("");
                            cm.setNota_min(min);
                            cm.setEvaluaciones(new_evaluaciones);
//                            if (evaluacion != null) {
...
//                            notapromedio = notapromedio + cm.getNota() + cm.getDps();
                    ...
                    for (int l = 0; l < cursomaterias.size(); l++) {
                        ...
                        c_m.setEvaluaciones(this.sigaa.getListaPlanEvaluacionesByIdCursoMateria(c_m.getId_curso_materia()));
                    ...
                    for (int h = 0; h < periodo_cursos.size(); h++) {
                        PeriodoCurso pc = (PeriodoCurso) periodo_cursos.get(h);
                        if (pc.getId_curso().equals(curso.getId_curso())) {
                            curso.setFecRegNotas(pc.getFecnotas());
                        }
                    }
                    retorno.put("curso", curso);
                    if (opcion != null) {
                        if (opcion.equals("_imprimeExcel")) {
                            String excel = this.centralizadorExcel(curso, id_gestion);
...
//                curso.setId_gestion(Integer.parseInt(id_gestion));
    ...
    private String centralizadorExcel(Curso curso, String id_gestion) {
        ...
        String dirPlantilla = System.getenv("SIGAA_HOME1") + "/documentos/plantillas_excel/plantilla_cetralizador_detallado.xls";
        ...
        try {
            ...
            sheet.addCell(new Label(3, 5, CadenaFecha, arial8));
            /*cant evas*/
            int colspan = 0;
            int val = 4;
            for (int j = 0; j < curso.getCursomaterias().size(); j++) {
                CursoMateria cm = (CursoMateria) curso.getCursomaterias().get(j);
                colspan = cm.getEvaluaciones().size()+1;
                sheet.addCell(new Label(val, 8, cm.getId_materia(), arial8green)); 
                val = val + 3;
            }
...
//                                                <% colspan++;%>
//                                            </c:forEach>
//                                            <td class="gridHeaders" colspan="<% out.print(colspan + 1);%>" title="${item_cm.materia}">${item_cm.id_materia}</td>
//                                        </c:forEach>
//            int val = 4;
//            for (int j = 0; j < curso.getCursomaterias().size(); j++) {
//                CursoMateria cm = (CursoMateria) curso.getCursomaterias().get(j);
...
//            }
            ...
            for (i = 0; i < curso.getEstudiantes().size(); i++) {
                ...
                for (int j = 0; j < est.getCursomaterias().size(); j++) {
                    ...
                    if (!curso.getId_curso().equals("K")&&!curso.getId_curso().equals("K1")) {
